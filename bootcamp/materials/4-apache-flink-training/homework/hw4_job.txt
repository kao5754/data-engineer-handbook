...
def create_aggregated_events_ip_sink_postgres(t_env):
    ...
    sink_ddl = f"""
        CREATE TABLE {table_name} (
            event_window_time_stamp TIMESTAMP(3),
            host VARCHAR,
            ip VARCHAR,
            num_hits BIGINT
        ) WITH (
...
def log_aggregation():
    ...
    try:
        ...
        # Create Aggregated Postgres Table
        aggregated_sink_table = create_aggregated_events_ip_sink_postgres(t_env)
    
        # Create a table from the source table with a session window of 5 minutes grouping by ip, host, window
        # Insert into Aggregated Postgres Table
